...
async def _play(self, ctx, *, query: str):
    """ Searches and plays a melody from a given query. """
...
async def _pause(self, ctx):
    """ Pauses/Resumes the current melody. """
    player = self.bot.lavalink.player_manager.get(ctx.guild.id)
...
async def _djremove(self, ctx, pos: int, member: discord.Member = None):
    """ Remove a melody from either the global queue or a users queue"""
    player = self.bot.lavalink.player_manager.get(ctx.guild.id)
...
async def _disconnect(self, ctx):
    """ Disconnects the player from the voice concert and clears its queue. """
    player = self.bot.lavalink.player_manager.get(ctx.guild.id)
...
async def _reconnect(self, ctx):
    """ Tries to disconnect then reconnect the player in case the bot gets stuck on a melody """
    player = self.bot.lavalink.player_manager.get(ctx.guild.id)
